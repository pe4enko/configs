...
#  Cancelling sudo takes half a minute.
    ...
    if [ $? -eq 0 ]; then
        ...
        for j in ${deldir[*]}; do
            for f in "$j/man/man1/*"; do
                ...
                sudo -- update-alternatives --remove $name $j/bin/$name
            done
            ...
            sudo -- update-alternatives --remove libnpjp2.so $j/jre/lib/i386/libnpjp2.so
       done
    fi
fi
...
NEWDIR=`$RADIO`;
sudo -D $COMPRESS_MSG -- gzip -9 $NEWDIR/man/man1/*.1 >/dev/null 2>&1 &
...
if [ -d "$NEWDIR/man/man1" ];then
    for f in $NEWDIR/man/man1/*; do
        ...
        if [ ! -f "/usr/bin/$name" -o -L "/usr/bin/$name" ]; then  
            ...
            sudo -- update-alternatives --install /usr/bin/$name $name $NEWDIR/bin/$name $LATEST --slave /usr/share/man/man1/$name.1.gz $name.1.gz $NEWDIR/man/man1/$name.1.gz
        fi
    done
    #File links without man pages
    ...
    for f in $NEWDIR/bin/*; do
        ...
        if [ ! -f "/usr/bin/$name" -o -L "/usr/bin/$name" ]; then  #some files, like jvisualvm might not be links
            sudo -- update-alternatives --install /usr/bin/$name $name $NEWDIR/bin/$name $LATEST
        fi
    done
fi
